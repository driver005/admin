import { ClaimItem, LineItem, Order, Return, Swap } from '@medusajs/medusa'
import {
    useAdminNotes,
...
function getLineItem(allItems: any[], itemId: string) {
    const line = allItems.find((line: { id: any }) => line.id === itemId)
...
function getReturnItems(allItems: any[], item: any) {
    const line = allItems.find((li: { id: any }) => li.id === item.item_id)
...
function getClaimItem(claimItem: ClaimItem) {
    return {
...
function getSwapItem(item: LineItem) {
    return {
        title: item.title,
        quantity: item.quantity,
...
function getWasRefundClaim(claimId: string, order: Order) {
    const claim = order.claims.find((c: { id: any }) => c.id === claimId)
