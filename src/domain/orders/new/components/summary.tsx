...
interface SummaryProps {
    ...
    customOptionPrice: string
    form: any
}

const Summary: React.FC<SummaryProps> = ({ items, showCustomPrice, customOptionPrice, form }) => {
    ...
    const handleAddDiscount = async () => {
        ...
        try {
            ...
            if (!data.discount.regions.find((d: any) => d.id === region.id)) {
                setDiscError(true)
            }
    ...
    return (
        <div className="min-h-[705px]">
                                    ...
                                    <span>
                                        {discount.rule.type === 'fixed'
                                            ? `${displayAmount(
                                                ...
                                                discount.rule.value
                                            )} ${region.country_code?.value.toUpperCase()}`
                                            : `${discount.rule.value} %`}
                                    </span>
                                </div>
                            )}
                ...
                <SummarySection title={'Shipping details'} editIndex={4}>
                    <div className="flex w-full">
                        <div className="border-r flex flex-col border-grey-20 pr-6">
                            ...
                            <span>
                                {`${shipping.postal_code} ${shipping.city} ${shipping.country_code.value.toUpperCase()}`}
                            </span>
                        </div>
                        <div className="pl-6 flex flex-col">
            ...
            <SummarySection title={'Billing details'} editIndex={4}>
                ...
                <span>
                    {`${billing.postal_code} ${billing.city} ${billing.country_code?.value.toUpperCase()}`}
                </span>
            </SummarySection>
        </div>
    )
...
const SummarySection: React.FC<SummarySectionProps> = ({ title, editIndex, children }) => {
