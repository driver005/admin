...
export const productToFormValuesMapper = (product: any) => {
    let thumbnail = product?.images?.length
        ? product.images.findIndex((img: any) => img.url === product.thumbnail)
        : 0
    ...
    return {
        ...
        type: product?.type
            ...
            : null,
        tags: product?.tags ? product.tags.map((t: any) => t.value) : [],
...
export const formValuesToCreateProductMapper = (
    ...
    if (isFeatureEnabled('sales_channels')) {
        scData['sales_channels'] = values.sales_channels.map(
            (salesChannel: any) => ({
                id: salesChannel.id,
            })
        )
    ...
    if (viewType === SINGLE_PRODUCT_VIEW) {
        values.variants = [
            {
                ...
                prices: values?.prices ? values.prices.map((p: any) => p.price) : [],
        ...
        values.variants = values?.variants.map((v: any) => ({
            title: v.title,
            sku: v.sku || null,
            ean: v.ean || null,
            inventory_quantity: v?.inventory_quantity
                ...
                : 0,
            prices: [],
            options: v.options.map((o: any) => ({ value: o })),
        }))
        values.options = values.options.map((o: any) => ({ title: o.name }))
    }
    ...
    return {
        ...
        tags: values?.tags ? values.tags.map((tag: any) => ({ value: tag })) : [],
...
export const formValuesToUpdateProductMapper = (values: any, isFeatureEnabled: any) => {
    const scData: any = {}
