export const formValuesToUpdateGiftCardMapper = (values: any) => {
    const payload = {
        ...values,
    ...
    if (values.tags) {
        payload['tags'] = values.tags.map((tag: any) => ({ value: tag }))
    }

    return payload
}

export const giftCardToFormValuesMapper = (giftCard: any) => {
    let thumbnail = giftCard?.images?.length
        ? giftCard.images.findIndex((img: any) => img.url === giftCard.thumbnail)
        : 0
    ...
    return {
        ...
        type: giftCard?.type
            ...
            : null,
        tags: giftCard?.tags ? giftCard.tags.map((t: any) => t.value) : [],
        ...
        prices: giftCard?.variants.length
            ? giftCard.variants[0].prices.map((price: any) => ({
                    price: {
