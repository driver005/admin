...
const RegionDetails: React.FC<RouteComponentProps & { id?: any, onDelete?: any, handleSelect?: any }> = ({ id, onDelete, handleSelect }) => {
    ...
    useEffect(() => {
        ...
        setPaymentOptions(
            payment_providers.map((c: any) => paymentProvidersMapper(c.id))
        )
    }, [payment_providers])
    ...
    useEffect(() => {
        ...
        setFulfillmentOptions(
            fulfillment_providers.map((c: any) => fulfillmentProvidersMapper(c.id))
        )
    ...
    useEffect(() => {
        ...
        setFulfillmentProviders(
            region.fulfillment_providers.map((v) =>
                fulfillmentProvidersMapper(v.id)
            )
        )
    }, [region])

    const getCurrencies = (storeCurrencies: any) => {
        const currs = storeCurrencies
            .filter((item: any) => item.code !== region?.currency_code)
            .map((el: any) => el.code)
    ...
    const handleFulfillmentChange = (values: any) => {
        const providers = values.map((v: any) => ({ value: v.value }))
    ...
    const handleDuplicate = () => {
        ...
        const payload = {
            ...
            fulfillment_providers: region.fulfillment_providers.map(
                (f) => f.id
            ),
        ...
        createRegion.mutate(payload, {
            onSuccess: ({ region }) => {
                notification(
                    'Success',
                    'Successfully duplicated region',
                    'success'
                )
